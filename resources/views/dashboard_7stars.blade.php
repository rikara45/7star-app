<x-app-layout>
    ...
    <div x-data="{ show: false, message: '' }" 
         ...
         class="fixed bottom-5 right-5 z-50" style="display: none;" x-show="show"
                ...
                <div class="p-8 relative z-10">
                    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div>
                            ...
                            <p class="text-gray-500 mt-2 text-sm max-w-2xl">Langkah pertama adalah menilai diri sendiri secara jujur. Skor ini akan digabungkan dengan penilaian atasan dan sejawat.</p>
                        </div>
                        <div>
                            @if($selfRequest)
                                @if($selfRequest->is_completed)
                                    <div class="flex items-center bg-green-50 border border-green-200 text-green-700 px-5 py-3 rounded-lg shadow-sm">
                                        ...
                                        <div><p class="font-bold text-sm">Sudah Mengisi</p><p class="text-xs opacity-80">Terima kasih.</p></div>
                                    </div>
                                @else
                                    <a href="{{ route('assessment.show', $selfRequest->access_token) }}" target="_blank" class="group inline-flex items-center bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 transition shadow-md transform hover:-translate-y-0.5">
                                        Isi Kuesioner Sekarang <svg class="w-4 h-4 ml-2 group-hover:translate-x-1 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                    </a>
                                @endif
                            @else
                                <form action="{{ route('dashboard.invite') }}" method="POST">
                                    ...
                                    <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 transition shadow-md">Mulai Self Assessment</button>
                                </form>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-white shadow-lg sm:rounded-xl border border-gray-100 h-full">
                        <div class="p-6 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                            <h3 class="text-lg font-bold text-gray-800 flex items-center">
                                <span class="bg-purple-100 text-purple-600 w-8 h-8 rounded-full flex items-center justify-center mr-3 text-sm font-bold">2</span> Undang Penilai
                            </h3>
                        </div>
                        <div class="p-6">
                            <p class="text-sm text-gray-500 mb-6">Masukkan data Atasan atau Teman Sejawat.</p>
                            @if(session('success')) <div class="mb-4 bg-green-50 border-l-4 border-green-500 text-green-700 p-3 text-xs rounded-r font-bold">{{ session('success') }}</div> @endif
                            @if(session('error')) <div class="mb-4 bg-red-50 border-l-4 border-red-500 text-red-700 p-3 text-xs rounded-r font-bold">{{ session('error') }}</div> @endif
                            ...
                            <form action="{{ route('dashboard.invite') }}" method="POST" class="space-y-5">
                                ...
                                <div>
                                    ...
                                    <select name="relationship" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                        <option value="superior" {{ $countSuperior >= 2 ? 'disabled' : '' }}>Atasan ({{ $countSuperior }}/2) {{ $countSuperior >= 2 ? '- Penuh' : '' }}</option>
                                        <option value="peer" {{ $countPeer >= 5 ? 'disabled' : '' }}>Sejawat ({{ $countPeer }}/5) {{ $countPeer >= 5 ? '- Penuh' : '' }}</option>
                                    </select>
                                </div>
                                <button type="submit" class="w-full bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 rounded-lg shadow transition transform hover:-translate-y-0.5">Generate Undangan</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white shadow-lg sm:rounded-xl border border-gray-100 h-full flex flex-col">
                        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                            <h3 class="text-lg font-bold text-gray-800">Daftar Penilai</h3>
                            <span class="text-xs bg-gray-100 text-gray-500 px-3 py-1 rounded-full font-bold">Total: {{ $requests->where('relationship', '!=', 'self')->count() }}</span>
                        </div>
                            ...
                            <table class="min-w-full text-sm text-left text-gray-500">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50 rounded-lg">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 rounded-r-lg text-center">Link / Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @forelse($requests as $req)
                                        ...
                                        <tr class="hover:bg-gray-50 transition duration-150 group">
                                            <td class="px-6 py-4 font-medium text-gray-900">
                                                <div class="flex items-center">
                                                    <div class="h-9 w-9 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-sm uppercase {{ $req->relationship == 'superior' ? 'bg-purple-500' : 'bg-orange-400' }}">{{ substr($req->assessor_name, 0, 1) }}</div>
                                                    <div class="ml-3"><div class="font-bold text-sm">{{ $req->assessor_name }}</div><div class="text-xs text-gray-400">{{ $req->assessor_email }}</div></div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                @if($req->relationship == 'superior') <span class="bg-purple-100 text-purple-700 text-xs px-2.5 py-0.5 rounded-full font-bold border border-purple-200">Atasan</span>
                                                @else <span class="bg-orange-100 text-orange-700 text-xs px-2.5 py-0.5 rounded-full font-bold border border-orange-200">Sejawat</span> @endif
                                            </td>
                                            <td class="px-6 py-4">
                                                @if($req->is_completed)
                                                    <span class="inline-flex items-center text-green-600 font-bold text-xs bg-green-50 px-2 py-1 rounded-full border border-green-100"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Selesai</span>
                                                @else
                                                    <span class="inline-flex items-center text-gray-500 font-medium text-xs bg-gray-100 px-2 py-1 rounded-full"><svg class="w-3 h-3 mr-1 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Pending</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                @if($req->is_completed)
                                                    <span class="text-xs text-gray-400 italic">Data Terkunci</span>
                                                @else
                                                    <div class="flex items-center justify-center space-x-3">
                                                        <div class="relative flex justify-center group/copy">
                                                            <button onclick="copyToClipboard('{{ route('assessment.show', $req->access_token) }}')" class="text-indigo-500 hover:text-indigo-700 transition" title="Salin Link"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>
                                                        </div>
                                                        <form action="{{ route('invite.destroy', $req->id) }}" method="POST" onsubmit="return confirm('Batalkan undangan ini?');">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" class="text-red-400 hover:text-red-600 transition" title="Hapus Undangan"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                                        </form>
                                                    </div>
                                                @endif
                                            </td>
                                        ...
                                        @endif
                                    @empty
                                        <tr><td colspan="4" class="px-6 py-10 text-center text-gray-400">Belum ada penilai diundang.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border border-gray-100 p-6">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-gray-100 pb-4">
                    <div class="mb-4 md:mb-0">
                        <h3 class="text-xl font-bold text-gray-900 flex items-center">
                            <span class="bg-green-100 text-green-600 w-8 h-8 rounded-full flex items-center justify-center mr-3 text-sm font-bold">3</span> Portofolio (Evidence)
                        </h3>
                ...
                @if(session('success_portfolio')) <div class="mb-6 bg-green-50 text-green-700 px-4 py-3 rounded-lg border border-green-200 flex items-center"><svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>{{ session('success_portfolio') }}</div> @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    @foreach($categories as $catName => $catHint)
                        @php
                            ...
                            $boxClass = $uploaded ? ($data->status == 'verified' ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200') : 'bg-white border-gray-200 hover:border-indigo-300';
                            $iconColor = $uploaded ? ($data->status == 'verified' ? 'text-green-500' : 'text-blue-500') : 'text-gray-300';
                        @endphp
                        ...
                        <div class="border rounded-xl p-5 transition duration-200 shadow-sm {{ $boxClass }} relative group">
                            <div class="absolute top-4 right-4">
                                @if($uploaded)
                                    @if($data->status == 'verified')
                                        <span class="bg-green-200 text-green-800 text-[10px] font-bold px-2 py-1 rounded-full flex items-center"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Verified ({{ $data->score }})</span>
                                    @else
                                        <span class="bg-blue-200 text-blue-800 text-[10px] font-bold px-2 py-1 rounded-full flex items-center animate-pulse"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Pending ({{ $data->score }})</span>
                                    @endif
                                @else
                            ...
                            <div class="flex items-start mb-3 pr-20">
                                <div class="mr-3 mt-0.5 {{ $iconColor }}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $uploaded ? 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' : 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z' }}"></path></svg></div>
                                <div><h4 class="font-bold text-gray-800 text-sm">{{ $catName }}</h4><p class="text-xs text-gray-500 italic mt-1 leading-tight">{{ $catHint }}</p></div>
                            </div>
                            ...
                            <form action="{{ route('portfolio.store') }}" method="POST" enctype="multipart/form-data" class="mt-4">
                                ...
                                @if(!$uploaded)
                                    <div class="space-y-2">
                                        ...
                                        <button type="submit" class="w-full bg-white border border-indigo-200 text-indigo-600 text-xs font-bold py-1.5 rounded-md hover:bg-indigo-50 transition">Upload Bukti</button>
                                    </div>
                                @else
                                    <div class="text-xs bg-white bg-opacity-60 p-2 rounded border border-gray-100 mb-2">
                                        <a href="{{ Storage::url($data->file_path) }}" target="_blank" class="flex items-center text-indigo-600 font-bold hover:underline mb-1"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>Lihat File</a>
                                        <p class="text-gray-500 truncate">{{ $data->description ?? 'Tanpa keterangan' }}</p>
                                    </div>
                                    <details class="group/edit relative">
                                        <summary class="list-none text-[10px] text-gray-400 cursor-pointer hover:text-indigo-600 text-right flex justify-end items-center"><span>Ganti File?</span><svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></summary>
            ...
            <div class="mt-8 text-center mb-12 pb-10" x-data="{ isLoading: false, showConfirm: false, missingCount: {{ $missingCount }} }">
                ...
                <div x-show="showConfirm" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 backdrop-blur-sm" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="bg-white rounded-lg shadow-xl p-6 max-w-md text-left mx-4 transform transition-all" @click.away="showConfirm = false">
                        <h3 class="text-lg font-bold text-gray-900 mb-2">Konfirmasi Proses</h3>
    ...
    <script>
        function copyToClipboard(text) {
            navigator.clipboard.writeText(text).then(() => {
                window.dispatchEvent(new CustomEvent('notify', { detail: 'Link berhasil disalin!' }));
            }, (err) => {
