            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center space-x-3 w-full md:w-auto bg-gray-50 p-1.5 rounded-lg border border-gray-200">
                    ...
                    <a href="{{ route('jurusan.verifikasi.index', ['status' => 'pending']) }}" 
                       class="px-4 py-1.5 text-xs font-bold rounded-md transition shadow-sm {{ request('status') == 'pending' ? 'bg-yellow-500 text-white' : 'text-gray-500 hover:bg-gray-200' }}">
                        Perlu Verifikasi
                    </a>
                </div>
                    ...
                    <table class="min-w-full text-sm text-left text-gray-500 divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-indigo-50 transition duration-150 ease-in-out group">
                                ...
                                {{-- STATUS DOKUMEN (LOGIKA 3 WARNA) --}}
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @if($dosen->total_portfolios == 0)
                                    ...
                                    @elseif($dosen->pending_count > 0)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-800 border border-yellow-200 animate-pulse">
                                            <span class="w-2 h-2 bg-yellow-500 rounded-full mr-2"></span>
                                            {{ $dosen->pending_count }} Perlu Verifikasi
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        @else
                                            <span class="inline-flex items-center justify-center px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-400 uppercase cursor-not-allowed">
                                                Menunggu Upload
                                            </span>
                                        @endif

                                        {{-- 2. Tombol HASIL (DIPERBAIKI) --}}
                                        @php
                                            $hasResult = $dosen->aiAnalyses->isNotEmpty();
                                        @endphp

                                        @if($hasResult)
                                            {{-- Muncul jika SUDAH ada hasil --}}
                                        ...
                                        @else
                                            {{-- Muncul jika BELUM ada hasil (ABU-ABU) --}}
